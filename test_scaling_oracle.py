...
def train_model(model, dataloader, epochs, device):
    model.train()
    # Learning rate is 0.01 (Note: comment says 0.001 but code is 0.01)
...
def train_enhanced(model, external_models, dataloader, epochs, device):
    ...
    # Adjust max_workers based on the number of external models, with a heuristic limit
...
class EnhancedScalingCNN(nn.Module):
    def __init__(self, num_external=0):
        ...
        # Safely get the feature dim from a dummy external model instance if possible
        try:
            dummy_model = CNN()
            dummy_model.eval() # Ensure layers are in correct mode for feature extraction
            external_feature_dim = dummy_model.get_features(torch.randn(1, 3, 32, 32)).size(1)
            del dummy_model # Clean up dummy model
        except Exception:
             # Fallback if get_features is not easily runnable or has different structure
             external_feature_dim = 512 # Default assumption
             print(f"Warning: Could not determine external feature dimension from CNN(). Assuming {external_feature_dim}")
...
def load_or_train(model, filepath, train_fn, *train_args):
    if os.path.exists(filepath):
        ...
        # Load state dict with map_location to handle potential device mismatches
        ...
        target_device = train_args[-1] if train_args else torch.device("cpu")
        model.to(target_device)
        # Pass args excluding the initial model argument for the train_fn call
        train_fn(model, *train_args[1:]) # Pass all args after the model itself to train_fn
...
def main():
    ...
    # load_or_train args: model, filepath, train_fn, train_fn_args... (dataloader, epochs, device)
    base_model = load_or_train(base_model, base_model_file, train_model, base_loader, pretrain_epochs, device)
    ...
    for i in range(8):
        ...
        # load_or_train args: model, filepath, train_fn, train_fn_args... (dataloader, epochs, device)
        model_ext = load_or_train(model_ext, ext_file, train_model, loader_ext, pretrain_epochs, device)
    ...
    for i in range(8):
        ...
        # load_or_train args: model, filepath, train_fn, train_fn_args... (external_models_list, dataloader, epochs, device)
        # Need to pass the correct subset of external models for training
        model_enh = load_or_train(model_enh, enh_file, train_enhanced, external_models[:i+1], enh_loader, enhanced_epochs, device)
    ...
    for i in range(8):
        ...
        # Note: FeatureDatasetConcat needs external models and device during initialization
        ...
        sample_feat, _ = next(iter(concat_loader)) # Get a sample from the loader
        input_dim = sample_feat.size(1) # Correctly get dimension from batch
        model_naive = NaiveClassifier(input_dim=input_dim).to(device)
        naive_file = f"./model_scaling_oracle/naive{i+1}.pt"
        # load_or_train args: model, filepath, train_fn, train_fn_args... (dataloader, epochs, device)
        model_naive = load_or_train(model_naive, naive_file, train_naive, concat_loader, enhanced_epochs, device)
    ...
    # Save the results. (Filename updated to scaling_oracle.json)
    print("\nSaving results...")
    with open("./results_ablate/scaling_oracle_test.json", "w") as f:
