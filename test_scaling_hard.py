...
class EnhancedScalingCNN(nn.Module):
    def __init__(self, num_external=0):
        ...
        # Assume CNN get_features outputs 512-dim vector
        feature_dim_cnn = 64 * 4 * 4 # Correct dimension after conv layers
        self.fc_layers = nn.Sequential(
            nn.Linear(feature_dim_cnn, 512),
            nn.ReLU(),
            nn.Dropout(0.5)
        )
        # Assumes external models' get_features also output 512-dim vector
        external_feature_dim = getattr(CNN(), 'feature_dim', 512) # Use feature_dim if defined in CNN, else assume 512
...
def evaluate_naive(model, dataloader, device):
    ...
    with torch.no_grad():
        for feats, labels in dataloader:
            ...
            all_labels.extend(labels.cpu().numpy())
            all_preds.extend(predicted.cpu().numpy())
            # FIX: Add .cpu() before .numpy() as probs is on the device
            all_probs.extend(probs.cpu().numpy())
    accuracy = correct / total_samples
    f1 = f1_score(all_labels, all_preds, average='weighted')
    all_labels_binarized = label_binarize(all_labels, classes=list(range(10)))
...
def load_or_train(model, filepath, train_fn, *train_args):
    if os.path.exists(filepath):
        ...
        model.to(train_args[-1]) # last arg is device
        model.load_state_dict(torch.load(filepath, map_location=train_args[-1]))
    else:
        print(f"{filepath} not found. Training model...")
        model.to(train_args[-1]) # Ensure model is on device for training
        train_fn(model, *train_args[1:-1], train_args[-1]) # Pass args excluding model and including device correctly
...
def main():
    ...
    # External datasets: create 8 subsets, all corrupted.
    external_datasets = []
    for i in range(8):
    ...
    # --- ADDITION START ---
    # Evaluate Ensemble Baselines (scaling by number of external models included).
    ...
    # Save the results.
    print("\nSaving results...")
    with open("./results_ablate/scaling_hard_test.json", "w") as f:
