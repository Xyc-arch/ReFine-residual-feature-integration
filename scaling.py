...
def load_external_subset(trainset, start_idx, num_samples):
    # Returns a subset of the training set from a given start index.
...
def load_enhanced_subset(trainset, available_indices, seed, num_samples):
    # Randomly select num_samples from available_indices using the provided seed.
    rng = np.random.RandomState(seed)
    chosen = rng.choice(available_indices, size=num_samples, replace=False)
    return Subset(trainset, chosen)

def load_test_sets():
    # Divide the CIFAR-10 test set into 5 subsets.
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5))
    ])
...
def train_enhanced(model, external_models, dataloader, epochs, device):
    # Training loop for the enhanced model using external feature extraction concurrently.
    model.train()
    optimizer = optim.Adam(model.parameters(), lr=0.001)
    criterion = torch.nn.CrossEntropyLoss()
...
def main():
    ...
    # Reserve the first 2000*num_external_models samples for external training.
    external_total = external_train_size * num_external_models
    external_indices_all = np.arange(0, external_total)
    
    # For enhanced training, use the remaining samples.
    ...
    for idx in range(num_external_models):
        ...
        model_path = f"./model/external{idx+1}.pt"
        torch.save(model_ext.state_dict(), model_path)
        external_models.append(model_ext)
    
    # ---- Train Enhanced Models ----
    enhanced_models = []
    # We want enhanced models with external representation counts from 0 up to num_external_models.
    for idx in range(num_external_models + 1):
        ...
        # For enhanced training, sample 2000 examples from available_indices using a seed offset.
        ...
        model_path = f"./model/enhanced{idx}.pt"
    ...
    print("Scaling law experiment completed. Results saved to ./results/scaling.json")
